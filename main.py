...
word_list = ['a', 'ability', 'able', 'about', 'above', 'accept', 'according', 'account', 'across', 'act', 'action',
             ...
             'hand', 'hang', 'happen', 'happy', 'hard', 'have', 'he', 'head', 'health', 'hear', 'heart', 'heat',
             'heavy', 'help', 'her', 'here', 'herself', 'high', 'him', 'himself', 'his', 'history', 'hit', 'hold',
             'home', 'hope', 'hospital', 'hot', 'hotel', 'hour', 'house', 'how', 'however', 'huge', 'human', 'hundred',
             ...
             'none', 'nor', 'north', 'not', 'note', 'nothing', 'notice', 'now', 'number', 'occur', 'of', 'off', 'offer',
             'office', 'officer', 'official', 'often', 'oh', 'oil', 'ok', 'old', 'on', 'once', 'one', 'only', 'onto',
             'open', 'operation', 'opportunity', 'option', 'or', 'order', 'organization', 'other', 'others', 'our',
             'out', 'outside', 'over', 'own', 'owner', 'page', 'pain', 'painting', 'paper', 'parent', 'part',
             'participant', 'particular', 'particularly', 'partner', 'party', 'pass', 'past', 'patient', 'pattern',
             ...
             'police', 'policy', 'political', 'politics', 'poor', 'popular', 'population', 'position', 'positive',
             'possible', 'power', 'practice', 'prepare', 'present', 'president', 'pressure', 'pretty', 'prevent',
             'price', 'private', 'probably', 'problem', 'process', 'produce', 'product', 'production', 'professional',
             'professor', 'program', 'project', 'property', 'protect', 'prove', 'provide', 'public', 'pull', 'purpose',
             'push', 'put', 'quality', 'question', 'quickly', 'quite', 'race', 'radio', 'raise', 'range', 'rate',
             ...
             'suffer', 'suggest', 'summer', 'support', 'sure', 'surface', 'system', 'table', 'take', 'talk', 'task',
             'tax', 'teach', 'teacher', 'team', 'technology', 'television', 'tell', 'ten', 'tend', 'term', 'test',
             'than', 'thank', 'that', 'the', 'their', 'them', 'themselves', 'then', 'theory', 'there', 'these', 'they',
             'thing', 'think', 'third', 'this', 'those', 'though', 'thought', 'thousand', 'threat', 'three', 'through',
             ...
             'trial', 'trip', 'trouble', 'true', 'truth', 'try', 'turn', 'two', 'type', 'under', 'understand',
...
# Random word list
def testing_text():
    test_list = random.sample(word_list, 50)
    return test_list


# Text input string
test_str = " ".join(testing_text())


# Check words per minute
def check():
    check_txt = output_txt.get(1.0, END).split()
    print(check_txt)
    test_list = input_txt.get(1.0, END).split()
    compare = set(test_list).intersection(check_txt)
    word_count = math.floor(len(compare)/60 * 100) 
    messagebox.showinfo(title="Stop", message="Times up, stop typing!")
    output_txt.delete(1.0, END)
    output_txt.insert(END, f"You have averaged {word_count} words per minute.")


# Timer mechanism
def countdown():
    timeout = 60
    count_time = Timer(timeout, check)
    count_time.start()
...
def clear_text():
    ...
    output_txt.delete(1.0, END)
    new_str = " ".join(testing_text())
    input_txt.insert(END, new_str)


# Create a Text Field
title = Label(text="How Fast Can You Type?")
f1 = Frame(window, background="white", height=5, width=30)
input_txt = Text(window, height=8, width=60, bg="#FFEEEE")
input_txt.insert(END, test_str)
input_txt.tag_configure("center", justify="center")
input_txt.tag_add("center", "1.0", "end")
output_txt = Text(window, height=8, width=60, bg="#CAF7E3")

# Buttons
start_btn = Button(f1, height=2, width=10, text="Start", command=countdown)
reset_btn = Button(f1, height=2, width=10, text="Clear", command=clear_text)
